...
local requestsDisabled = true --getgenv and getgenv().DISABLE_Heistware_REQUESTS
local InterfaceBuild = '3K3W'
local Release = "Build 1.68"
local HeistwareFolder = "Heistware"
local ConfigurationFolder = HeistwareFolder.."/Configurations"
local ConfigurationExtension = ".rfld"
local settingsTable = {
	General = {
		-- if needs be in order just make getSetting(name)
		HeistwareOpen = {Type = 'bind', Value = 'K', Name = 'Heistware Keybind'},
		-- buildwarnings
		-- Heistwareprompts
...
local overriddenSettings: { [string]: any } = {} -- For example, overriddenSettings["System.HeistwareOpen"] = "J"
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			if isfolder and isfolder(HeistwareFolder) then
				if isfile and isfile(HeistwareFolder..'/settings'..ConfigurationExtension) then
					file = readfile(HeistwareFolder..'/settings'..ConfigurationExtension)
				end
			end
			...
			if useStudio then
				file = [[
		{"General":{"HeistwareOpen":{"Value":"K","Type":"bind","Name":"Heistware Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"Heistware Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
	]]
			end
	...
	if not success then 
		if writefile then
			warn('Heistware had an issue accessing configuration saving capability.')
		end
	end
end
...
if not requestsDisabled then
	...
	if cachedSettings and (#cachedSettings == 0 or (cachedSettings.System and cachedSettings.System.usageAnalytics and cachedSettings.System.usageAnalytics.Value)) then
		sendReport("execution", "Heistware")
...
local Heistware = useStudio and script.Parent:FindFirstChild('Heistware') or game:GetObjects("rbxassetid://10804731440")[1]
local buildAttempts = 0
local correctBuild = false
local warned
local globalLoaded
local HeistwareDestroyed = false -- True when HeistwareLibrary:Destroy() is called

repeat
	if Heistware:FindFirstChild('Build') and Heistware.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('Heistware | Build Mismatch')
		print('Heistware may encounter issues as you are running an incompatible interface version ('.. ((Heistware:FindFirstChild('Build') and Heistware.Build.Value) or 'No Build') ..').\n\nThis version of Heistware is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	toDestroy, Heistware = Heistware, useStudio and script.Parent:FindFirstChild('Heistware') or game:GetObjects("rbxassetid://10804731440")[1]
	if toDestroy and not useStudio then toDestroy:Destroy() end

	buildAttempts = buildAttempts + 1
until buildAttempts >= 2

Heistware.Enabled = false

if gethui then
	Heistware.Parent = gethui()
elseif syn and syn.protect_gui then 
	syn.protect_gui(Heistware)
	Heistware.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	Heistware.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == Heistware.Name and Interface ~= Heistware then
			Interface.Enabled = false
			Interface.Name = "Heistware-Old"
		end
	end
elseif not useStudio then
...
if Heistware.AbsoluteSize.X < minSize.X and Heistware.AbsoluteSize.Y < minSize.Y then
...
local Main = Heistware.Main
local MPrompt = Heistware:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = Heistware:FindFirstChild('Drag')
...
Heistware.DisplayOrder = 100
LoadingFrame.Version.Text = Release

-- Thanks to Latte Softworks for the Lucide integration for Roblox
local Icons = useStudio and require(script.Parent.icons) or loadWithTimeout('https://raw.githubusercontent.com/SiriusSoftwareLtd/Heistware/refs/heads/main/icons.lua')
-- Variables
...
local Notifications = Heistware.Notifications

local SelectedTheme = HeistwareLibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = HeistwareLibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	Heistware.Main.BackgroundColor3 = SelectedTheme.Background
	Heistware.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	Heistware.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	Heistware.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	Heistware.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	Heistware.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	Heistware.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		Heistware.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		Heistware.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(Heistware:GetDescendants()) do
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("Heistware | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("Heistware | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('Heistware had an issue decoding the configuration file, please try delete the file and reopen Heistware.') return end

	-- Iterate through current UI elements' flags
	for FlagName, Flag in pairs(HeistwareLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("Heistware | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--HeistwareLibrary:Notify({Title = "Heistware Flags", Content = "Heistware was unable to find '"..FlagName.. "' in the save file. Check sirius.menu/discord for help.", Image = 3944688398})
		end
	end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(HeistwareLibrary.Flags) do
...
function HeistwareLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('Heistware | Not seeing your actions in notifications?')
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			HeistwareLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show'.", Duration = 7, Image = 4400697855})
		else
			HeistwareLibrary:Notify({Title = "Interface Hidden", Content = `The interface has been hidden, you can unhide the interface by tapping {getSetting("General", "HeistwareOpen")}.`, Duration = 7, Image = 4400697855})
		end
	end
...
local function saveSettings() -- Save settings to config file
	...
	if success then
		...
		if writefile then
			writefile(HeistwareFolder..'/settings'..ConfigurationExtension, encoded)
		end
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('Heistware Settings', 0, true)

	if TabList['Heistware Settings'] then
		TabList['Heistware Settings'].LayoutOrder = 1000
	end

	if Elements['Heistware Settings'] then
		Elements['Heistware Settings'].LayoutOrder = 1000
	end
...
function HeistwareLibrary:CreateWindow(Settings)
	print('creating window')
	if Heistware:FindFirstChild('Loading') then
		if getgenv and not getgenv().HeistwareCached then
			Heistware.Enabled = true
			Heistware.Loading.Visible = true

			task.wait(1.4)
			Heistware.Loading.Visible = false
		end
	end

	if getgenv then getgenv().HeistwareCached = true end

	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				HeistwareLibrary:Notify({Title = 'Build Mismatch', Content = 'Heistware may encounter issues as you are running an incompatible interface version ('.. ((Heistware:FindFirstChild('Build') and Heistware.Build.Value) or 'No Build') ..').\n\nThis version of Heistware is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end
	...
	if Settings.ToggleUIKeybind then -- Can either be a string or an Enum.KeyCode
		...
		if type(keybind) == "string" then
			...
			overrideSetting("General", "HeistwareOpen", keybind)
		elseif typeof(keybind) == "EnumItem" then
			assert(keybind.EnumType == Enum.KeyCode, "ToggleUIKeybind must be a KeyCode enum")
			overrideSetting("General", "HeistwareOpen", keybind.Name)
		else
			error("ToggleUIKeybind must be a string or KeyCode enum")
		end
	end

	if isfolder and not isfolder(HeistwareFolder) then
		makefolder(HeistwareFolder)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "Heistware"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "Heistware Interface Suite" then
		LoadingFrame.Version.Text = "Heistware UI"
	end
	...
	if not Settings.DisableHeistwarePrompts then
		task.spawn(function()
			while true do
				task.wait(math.random(180, 600))
				HeistwareLibrary:Notify({
					Title = "Heistware Interface",
	...
	if Settings.Discord and Settings.Discord.Enabled and not useStudio then
		if isfolder and not isfolder(HeistwareFolder.."/Discord Invites") then
			makefolder(HeistwareFolder.."/Discord Invites")
		end

		if isfile and not isfile(HeistwareFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if requestFunc then
				pcall(function()
					requestFunc({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(HeistwareFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"Heistware RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		if isfolder and not isfolder(HeistwareFolder.."/Key System") then
			makefolder(HeistwareFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("Heistware | "..Key.." Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Heistware specific development.')
				end
			end
		end
		...
		if isfile and isfile(HeistwareFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(HeistwareFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(HeistwareFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						HeistwareLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				-- Prevents animation from trying to play if the button's callback called HeistwareLibrary:Destroy()
				if HeistwareDestroyed then
					return
				end
				if not Success then
					...
					print("Heistware | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Heistware specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= HeistwareLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					HeistwareLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			Heistware.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("Heistware | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Heistware specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					HeistwareLibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("Heistware | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with Heistware specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					HeistwareLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("Heistware | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with Heistware specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					HeistwareLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= HeistwareLibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Heistware | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Heistware specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						HeistwareLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("Heistware | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('Check docs.sirius.menu for help with Heistware specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					HeistwareLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			HeistwareLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			HeistwareLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('Heistware had an issue creating settings.') end
...
function HeistwareLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function HeistwareLibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function HeistwareLibrary:Destroy()
	HeistwareDestroyed = true
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['Heistware Settings'])
		end)
	end)
...
hideHotkeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode[getSetting("General", "HeistwareOpen")]) and not processed then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
...
function HeistwareLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				HeistwareLibrary:Notify({Title = "Heistware Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			HeistwareLibrary:Notify({Title = "Heistware Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('Heistware Configurations Error | '..tostring(result))
			HeistwareLibrary:Notify({Title = "Heistware Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
if useStudio then
	...
	--local Window = HeistwareLibrary:CreateWindow({
	--	Name = "Heistware Example Window",
	--	LoadingTitle = "Heistware Interface Suite",
	--	Theme = 'Default',
	--	Icon = 0,
	...
	--		FileName = "Key", -- It is recommended to use something unique as other scripts using Heistware may overwrite your key file
	--		SaveKey = true, -- The user's key will be saved, but if you change the key, they will be unable to use your script
	--		GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like Heistware to get the key from
	...
	----HeistwareLibrary:Notify({Title = "Heistware Interface", Content = "Welcome to Heistware. These - are the brand new notification design for Heistware, with custom sizing and Heistware calculated wait times.", Image = 4483362458})
	...
	--local thoptions = {}
	--for themename, theme in pairs(HeistwareLibrary.Theme) do
...
task.delay(4, function()
	HeistwareLibrary.LoadConfiguration()
